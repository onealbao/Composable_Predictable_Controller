...
if (round_num > 0)
     if (round_num == 1)
        ...
        prev_e = [buffer(round_num) 200 200]';
         
     elseif  (round_num == 2)
         prev_e = [buffer(round_num - 1) buffer(round_num) 200]';
     elseif  (round_num == 3)
         prev_e = [buffer(round_num - 2) buffer(round_num - 1) buffer(round_num)]';
         %prev_e = ein1(rc_CTIME-pred_span+1:rc_CTIME);
     else
         prev_e = buffer(round_num-2:round_num);
