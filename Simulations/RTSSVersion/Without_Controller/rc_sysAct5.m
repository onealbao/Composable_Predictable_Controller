...
if rc_CTIME == 0,
    ...
    % output signal
    next_x = [next_q next_y adjust_u next_ec read_ar v(1) e5(rc_CTIME) ];
    ...
    q  = next_q;
    y  = next_y;
    ec = alpha * alpha*782.9*782.9*782.9*782.9*4;
  ...
  costMatrix = -(x1)/(2000*2000*2000*2000*alpha*alpha) + (x2)/(2000*2000)-x3/774/2*7/2000;
  if costMatrix < 0
    costMatrix = 0;
end
