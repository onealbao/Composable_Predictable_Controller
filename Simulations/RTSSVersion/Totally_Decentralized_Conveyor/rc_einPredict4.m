...
function est_ein = rc_einPredict4()

global rc_LOOKAHEAD_STEPS   % prediction horizon
global ein4                 % environment input
...
if (round_num4 > 0)
    if (round_num4 == 1)
       ...
       prev_e = [buffer4(round_num4) 200 200]';
        
    elseif  (round_num4 == 2)
        prev_e = [buffer4(round_num4 - 1) buffer4(round_num4) 200]';
    elseif  (round_num4 == 3)
        prev_e = [buffer4(round_num4 - 2) buffer4(round_num4 - 1) buffer4(round_num4)]';
        %prev_e = ein4(rc_CTIME-pred_span+1:rc_CTIME);
    else
        prev_e = buffer4(round_num4-2:round_num4);
